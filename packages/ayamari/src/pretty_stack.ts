...
export class PrettyStack {
  ...
  static #AYAMARI_ERR_KEYS = new Set([
    'code',
    'levelValue',
    'createdAt',
  ]);
  ...
  static print(
    ...
    const parts = chain.map((err) => {
      ...
      const header = PrettyStack.#formatHeader(
        lines[0] ?? '',
        c,
      );
      const frames = PrettyStack.#processFrames(
        lines,
        seen,
        cwd,
        c,
      ...
      const extras = PrettyStack.#formatExtraProps(
        err,
        c,
        sensitive,
      );
  ...
  static #parseFrame(line: string): ParsedFrame | null {
    const match = PrettyStack.#FRAME_RE.exec(line);
    if (match) {
      const file = match[2];
      if (file === undefined) {
        return null;
      ...
      return {
        methodName:
          match[1] ?? PrettyStack.#UNKNOWN_FUNCTION,
        file,
        lineNumber: match[3],
        column: match[4],
      };
    }
    const anon = PrettyStack.#ANON_FRAME_RE.exec(line);
    if (anon) {
      return {
        methodName:
          anon[1] ?? PrettyStack.#UNKNOWN_FUNCTION,
   ...
   *          → "fastify"
   */
  static #extractPackageFromPath(
    path: string,
  ): string | null {
    const pnpmMatch = PrettyStack.#PNPM_PKG_RE.exec(path);
    if (pnpmMatch?.[1]) {
    ...
    const npmMatch = PrettyStack.#NPM_PKG_RE.exec(path);
    if (npmMatch?.[1]) {
      return npmMatch[1];
    }
    return null;
  ...
  static #isAyamariErr(err: AyamariErr | Error): boolean {
    return 'levelValue' in err;
  }

  static #formatExtraProps(
    err: AyamariErr | Error,
    c: Palette,
    sensitiveKeys: Set<string>,
  ): string[] {
    const result: string[] = [];
    const skipAyamariKeys = PrettyStack.#isAyamariErr(err);
    for (const key of Object.keys(err)) {
      if (PrettyStack.#STANDARD_ERROR_KEYS.has(key)) {
        continue;
      }
      if (
        skipAyamariKeys &&
        PrettyStack.#AYAMARI_ERR_KEYS.has(key)
      ) {
        continue;
      }
  ...
  static #processFrames(
    ...
    for (const line of lines.slice(1)) {
      const frame = PrettyStack.#parseFrame(line);
      ...
      seen.add(line);
      const pkg = PrettyStack.#extractPackageFromPath(
        frame.file,
      );
      if (pkg) {
        if (pkg !== currentPkg) {
        ...
        flushGroup();
        result.push(
          PrettyStack.#formatFrame(frame, cwd, c),
        );
      }
    }
    flushGroup();
